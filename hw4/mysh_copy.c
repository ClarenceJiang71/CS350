...
int main(int argc, char const *argv[])
{
    while(1){
        ...
        printf("Check R\n");
        //Execuate
        // pid_t * pid_list = malloc(sizeof(pid_t));
        pid_t * pid_list;
        for (int i=0; command_list[i]; i++){
            pid_t pid = fork();
            ...
            //parent case
                ...
                if (!background_check){
                    printf("backgroudn check\n");
                    count_foreground++;
                    if (count_foreground == 1){
                        // pid_list = malloc(sizeof(pid_t));
                        pid_list = calloc(1, sizeof(pid_t));
                        pid_list[0] = pid;
                    }
                    else{
                        pid_list = realloc(pid_list, sizeof(pid_t)*count_foreground);
                        pid_list[count_foreground-1] = pid;
                        pid_list[count_foreground] = NULL;
                    }
                    
                    // // collect all of the foreground pid
                    // pid_list[count_foreground-1] = pid;
                    // // pid_list[count_foreground] = NULL;
        ...
        while (pid_list!= NULL){
            printf("Check here aa\n");
            // int pid_list_size = sizeof(pid_list)/sizeof(pid_list[0]);
            int found = 0;
            pid_t pid_target = wait(&status);
            if (pid_target == -1){
                break;
            }
            else if(pid_target>0){
                // for (int i=0; pid_list[i]; i++){
                //     if ((found = 1) && (i<count_foreground)){
                //         pid_list[i-1] = pid_list[i];
                //         continue;
                //     }
                //     if (pid_list[i]==pid_target){
                //         found = 1;
                //     }
                // }
                for (int i=0; pid_list[i]; i++){
                ...
                for (int j = found; j<count_foreground-1; j++){
                    pid_list[j] = pid_list[j+1];
                }
                pid_list = realloc(pid_list, sizeof(pid_t *)*(count_foreground-1));
                count_foreground--;
                // pid_list = tmp;
    ...
    // // for (int i=0; command_list[i]; i++){
    // //     struct command * com = command_list[i];
    // //     free(comm);
    // // }
    // free(command_list);
    }
